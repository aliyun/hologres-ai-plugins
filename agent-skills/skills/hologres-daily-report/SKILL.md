...
按以下 7 个步骤（Step 0 ~ Step 6）依次执行实例确认、数据采集和诊断分析。每一步的详细 SQL 和命令见 `references/` 目录。

### Step 0: 实例信息确认回复

在开始数据采集之前，先确认并回复用户将要诊断的实例信息。

#### 0.1 确定 instance_id

- 如果用户在请求中明确指定了实例 ID，使用用户指定的值
- 如果用户未指定，使用当前 profile 的默认 `instance_id`

#### 0.2 回复确认

向用户回复确认将要使用的实例信息，回复格式：

```
好的，我将为实例 {instance_id} 生成运维诊断报告。
```

> **注意**：确认回复后直接进入 Step 1 开始数据采集，无需等待用户二次确认。
...
1. 先执行 Step 0（实例信息确认回复），向用户确认将要诊断的实例名称
2. 执行 Step 1（实例基础信息），确认实例可连接
3. 并行执行 Step 2（计算资源）和 Step 3（SQL 诊断）和 Step 4（存储成本）
4. Step 5（容量预测）依赖 Step 2 和 Step 4 的结果
5. Step 6（评分与报告生成）汇总所有结果
